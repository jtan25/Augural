...
// Create a new Market GraphQL entity
export function handleMarketCreated(event: MarketCreatedEvent): void {
  let market = new Market(
    event.transaction.hash.toHex() + "-" + event.logIndex.toString()
  )

  // Keeps track of order count, total bid amount
  market.orders = 0
  market.totalBid = BigInt(0)

  // Store keywords for React component market search feature
  market.keywords = market.description.split(' ')
